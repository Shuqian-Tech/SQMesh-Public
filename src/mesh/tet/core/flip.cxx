...
void TetMeshCore::flippush(badface*& fstack, triface* flipface)
{
...
void TetMeshCore::flip23(triface* fliptets, int hullflag, flipconstraints *fc)
{
  ...
  if (fc->remove_ndelaunay_edge) { // calc_tetprism_vol
    double volneg[2], volpos[3], vol_diff;
    if (pd != dummypoint) {
      if (pc != dummypoint) {
...
void TetMeshCore::flip32(triface* fliptets, int hullflag, flipconstraints *fc)
{
  triface topcastets[3], botcastets[3];
  triface newface, casface;
  face flipshs[3]; 
  ...
  if (fc->remove_ndelaunay_edge) { // calc_tetprism_vol
    double volneg[3], volpos[2], vol_diff;
    if (pc != dummypoint) {
      if (pd != dummypoint) {
...
void TetMeshCore::flip41(triface* fliptets, int hullflag, flipconstraints *fc)
{
  ...
  if (fc->remove_ndelaunay_edge) { // calc_tetprism_vol
    double volneg[4], volpos[1], vol_diff;
    if (dummyflag > 0) {
      if (pa == dummypoint) {
        volneg[0] = 0.;
...
int TetMeshCore::flipnm(triface* abtets, int n, int level, int abedgepivot,
                       flipconstraints* fc)
{
...
int TetMeshCore::flipnm_post(triface* abtets, int n, int nn, int abedgepivot,
                            flipconstraints* fc)
{
...
int TetMeshCore::insertpoint(point insertpt, triface *searchtet, face *splitsh,
...
void TetMeshCore::insertpoint_abort(face *splitseg, insertvertexflags *ivf)
{
  triface *cavetet;
  face *parysh;
  int i;
