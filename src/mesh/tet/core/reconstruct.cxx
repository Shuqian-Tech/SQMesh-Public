...
int TetMeshCore::search_face(point pi, point pj, point pk, triface &tetloop)
{
  pinfect(pi);
  pinfect(pj);
  pinfect(pk);
...
int TetMeshCore::search_edge(point p0, point p1, triface &tetloop)
{
  triface t;
  int ii;
...
void TetMeshCore::reconstructmesh()
{
  ...
  shellface sptr;
  point p[4], q[3];
  double ori, attrib, volume;
  double cosang_tol, cosang;
  double n1[3], n2[3];
  ...
  for (i = 0; i < in->numberoftetrahedra; i++) {
    ...
    if (b->varvolume) {
      if (in->tetrahedronvolumelist != (double *) NULL) {
  ...
  if (!b->nobisect || checkconstraints) {
    ...
    if (b->quality && (in->facetconstraintlist != (double *) NULL)) {
...
int TetMeshCore::scout_point(point searchpt, triface *searchtet, int randflag)
{
  if (b->verbose > 3) {
  ...
  if (loc == INTETRAHEDRON) {
    ...
    for (chktet.ver = 0; chktet.ver < 4; chktet.ver++) {
      if (issubface(chktet)) {
        ...
        point pb = org(chktet);
        point pc = org(chktet);
        double ori = orient3d(pa, pb, pc, searchpt);
        double averlen = (distance(pa, pb) +
                        distance(pb, pc) +
                        distance(pc, pa)) / 3.;
        double len3 = averlen * averlen * averlen;
        double ratio = (-ori) / len3;
        if (ratio < b->epsilon) {
          *searchtet = chktet;
          loc = ONFACE;
          break;
  ...
  if (loc == ONFACE) {
    ...
    for (int i = 0; i < 3; i++) {
      if (issubseg(chkface)) {
        double cosang = cos_interiorangle(searchpt, org(chkface), dest(chkface));
  ...
  if (loc == ONEDGE) {
    ...
    for (int i = 0; i < 2; i++) {
      double dd = distance(searchpt, org(chkedge));
      if (dd < minedgelength) {
        *searchtet = chkedge;
        loc = ONVERTEX;
        break;
...
double TetMeshCore::getpointmeshsize(point searchpt, triface *searchtet, int iloc)
{
  point *pts, pa, pb, pc;
  double volume, vol[4], wei[4];
  double size;
  int i;
...
void TetMeshCore::interpolatemeshsize()
{
  triface searchtet;
  point ploop;
...
void TetMeshCore::insertconstrainedpoints(point *insertarray, int arylen,
                                         int rejflag)
{
...
void TetMeshCore::insertconstrainedpoints(TetMeshData *addio)
{
...
void TetMeshCore::collectremovepoints(arraypool *remptlist)
{
  point ptloop, *parypt;
  verttype vt;
  ...
  if (b->coarsen_param > 0) { // -R1/#
    ...
    if (intptlist->objects > 0l) {
      ...
      int remcount = (int) ((double) intptlist->objects * b->coarsen_percent);
...
void TetMeshCore::meshcoarsening()
{
  arraypool *remptlist;
