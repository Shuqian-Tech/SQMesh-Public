...
int TetMeshCore::bondtbl[12][12] = {{0,},};
int TetMeshCore::enexttbl[12] = {0,};
int TetMeshCore::eprevtbl[12] = {0,};
int TetMeshCore::enextesymtbl[12] = {0,};
int TetMeshCore::eprevesymtbl[12] = {0,};
int TetMeshCore::eorgoppotbl[12] = {0,};
int TetMeshCore::edestoppotbl[12] = {0,};
int TetMeshCore::fsymtbl[12][12] = {{0,},};
int TetMeshCore::facepivot1[12] = {0,};
int TetMeshCore::facepivot2[12][12] = {{0,},};
int TetMeshCore::tsbondtbl[12][6] = {{0,},};
int TetMeshCore::stbondtbl[12][6] = {{0,},};
int TetMeshCore::tspivottbl[12][6] = {{0,},};
int TetMeshCore::stpivottbl[12][6] = {{0,},};

// Table 'esymtbl' takes an directed edge (version) as input, returns the
//   inversed edge (version) of it.

int TetMeshCore::esymtbl[12] = {9, 6, 11, 4, 3, 7, 1, 5, 10, 0, 8, 2};
...
int TetMeshCore:: orgpivot[12] = {7, 7, 5, 5, 6, 4, 4, 6, 5, 6, 7, 4};
int TetMeshCore::destpivot[12] = {6, 4, 4, 6, 5, 6, 7, 4, 7, 7, 5, 5};
int TetMeshCore::apexpivot[12] = {5, 6, 7, 4, 7, 7, 5, 5, 6, 4, 4, 6};
int TetMeshCore::oppopivot[12] = {4, 5, 6, 7, 4, 5, 6, 7, 4, 5, 6, 7};
...
int TetMeshCore::ver2edge[12] = {0, 1, 2, 3, 3, 5, 1, 5, 4, 0, 4, 2};
int TetMeshCore::edge2ver[ 6] = {0, 1, 2, 3, 8, 5};

// Edge versions whose apex or opposite may be dummypoint.

int TetMeshCore::epivot[12] = {4, 5, 2, 11, 4, 5, 2, 11, 4, 5, 2, 11};


// Table 'snextpivot' takes an edge version as input, returns the next edge
//   version in the same edge ring.

int TetMeshCore::snextpivot[6] = {2, 5, 4, 1, 0, 3};
...
int TetMeshCore::sorgpivot [6] = {3, 4, 4, 5, 5, 3};
int TetMeshCore::sdestpivot[6] = {4, 3, 5, 4, 3, 5};
int TetMeshCore::sapexpivot[6] = {5, 5, 3, 3, 4, 4};
...
void TetMeshCore::arraypool::restart()
{
  objects = 0l;
}
...
void TetMeshCore::arraypool::poolinit(int sizeofobject, int log2objperblk)
{
...
TetMeshCore::arraypool::arraypool(int sizeofobject, int log2objperblk)
{
  poolinit(sizeofobject, log2objperblk);
}

TetMeshCore::arraypool::~arraypool()
{
  int i;
...
char* TetMeshCore::arraypool::getblock(int objectindex)
{
  char **newarray;
  char *block;
  int newsize;
...
void* TetMeshCore::arraypool::lookup(int objectindex)
{
  char *block;
  int topindex;
...
int TetMeshCore::arraypool::newindex(void **newptr)
{
...
TetMeshCore::memorypool::memorypool()
{
...
TetMeshCore::memorypool::memorypool(int bytecount, int itemcount, int wsize, 
                                   int alignment)
{
...
void TetMeshCore::memorypool::poolinit(int bytecount,int itemcount,int wordsize,
                                      int alignment)
{
...
void TetMeshCore::memorypool::restart()
{
  uintptr_t alignptr;
...
void* TetMeshCore::memorypool::alloc()
{
  void *newitem;
  void **newblock;
  uintptr_t alignptr;
...
void TetMeshCore::memorypool::dealloc(void *dyingitem)
{
...
void TetMeshCore::memorypool::traversalinit()
{
  uintptr_t alignptr;
...
void* TetMeshCore::memorypool::traverse()
{
  void *newitem;
  uintptr_t alignptr;
...
void TetMeshCore::makeindex2pointmap(point*& idx2verlist)
{
  point pointloop;
  int idx;
...
void TetMeshCore::makepoint2submap(memorypool* pool, int*& idx2faclist,
                                  face*& facperverlist)
{
  face shloop;
  int i, j, k;
...
void TetMeshCore::tetrahedrondealloc(tetrahedron *dyingtetrahedron)
{
...
TetMeshCore::tetrahedron* TetMeshCore::tetrahedrontraverse()
{
...
TetMeshCore::tetrahedron* TetMeshCore::alltetrahedrontraverse()
{
  tetrahedron *newtetrahedron;
...
void TetMeshCore::shellfacedealloc(memorypool *pool, shellface *dyingsh)
{
...
TetMeshCore::shellface* TetMeshCore::shellfacetraverse(memorypool *pool)
{
...
void TetMeshCore::pointdealloc(point dyingpoint)
{
...
TetMeshCore::point TetMeshCore::pointtraverse()
{
  point newpoint;
...
void TetMeshCore::maketetrahedron(triface *newtet)
{
  ...
  //double *polar = get_polar(newtet->tet);
  //polar[4] = 0.0;
...
void TetMeshCore::maketetrahedron2(triface* newtet, point pa, point pb,
                                  point pc, point pd)
{
  newtet->tet = (tetrahedron *) tetrahedrons->alloc();
...
void TetMeshCore::makeshellface(memorypool *pool, face *newface)
{
...
void TetMeshCore::makepoint(point* pnewpoint, enum verttype vtype)
{
  int i;
...
void TetMeshCore::initializepools()
{
  ...
  if (b->metric) {
    ...
    if (bgm != (TetMeshCore *) NULL) {
      // A background mesh is allocated. It may not exist though.
      sizeoftensor = (bgm->in != (TetMeshData *) NULL) ? 
  ...
  if (useinsertradius) {
    // Increase a space (double) for saving point insertion radius, it is
  ...
  point2simindex = ((pointmtrindex + sizeoftensor) * sizeof(double)
  ...
  if (b->plc || b->refine /*|| b->voroout*/) {
    ...
    if (b->metric && (bgm != (TetMeshCore *) NULL)) {
  ...
  points = new memorypool(pointsize, b->vertexperblock, sizeof(double), 0);
  ...
  polarindex = (elesize + sizeof(double) - 1) / sizeof(double);
  // The index within each element at which its attributes are found, where
  //   the index is measured in REALs. 
  //elemattribindex = (elesize + sizeof(double) - 1) / sizeof(double);
  ...
  if (b->varvolume) {
    elesize = (volumeboundindex + 1) * sizeof(double);
  } else {
    elesize = volumeboundindex * sizeof(double);
  }
  ...
  if (b->plc || b->refine) { // if (b->useshelles) {
    ...
    //   found, where the index is measured in REALs.
    areaboundindex = (shsize + sizeof(double) - 1) / sizeof(double);
    // If -q switch is in use, increase the number of bytes occupied by
    //   a subface for saving maximum area bound.
    if (checkconstraints) { 
      shsize = (areaboundindex + 1) * sizeof(double);
    } else {
      shsize = areaboundindex * sizeof(double);
    }
